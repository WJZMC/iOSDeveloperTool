...
-(void)createNavViewTitle:(NSString *)title {
    UIView *nav = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SelfView_W, [UNApplicationTool getNav_StatusH])];
//    nav.backgroundColor = WPhoto_TopView_Color;
    nav.backgroundColor = [UIColor whiteColor];
    [self addSubview:nav];
    
    UILabel *titleLab = [[UILabel alloc]initWithFrame:CGRectMake(0, 20, 80, [UNApplicationTool getNav_StatusH]-20)];
    titleLab.text = title;
    ...
    titleLab.center = CGPointMake(nav.center.x, ([UNApplicationTool getNav_StatusH]-20)/2+20);
