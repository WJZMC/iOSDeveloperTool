...
/**
 下标固定高度 wj do
 */
@property (nonatomic, assign) CGFloat underLinefixedW;
...
- (UIFont *)titleSelFont
{
    if (_titleSelFont == nil) {
        _titleSelFont = YZTitleFont;
    }
...
-(UIButton*)addBtn
{
    if (_addBtn== nil) {
        ...
        [_addBtn setImage:[UIImage imageNamed:@"sy-add"] forState:UIControlStateNormal];
        [_addBtn addTarget:self action:@selector(AddBtnAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.titleScrollView addSubview:_addBtn];
    }
    return _addBtn;
}
...
-(void)setIsSHowAdd:(BOOL)isSHowAdd
{
    ...
    self.addBtn.hidden=!_isSHowAdd;
}

- (void)setIsScrollEnable:(BOOL)isScrollEnable {
    _isScrollEnable = isScrollEnable;
    
    if (_isScrollEnable == YES) {
        
    } else {
        self.titleScrollView.contentSize = CGSizeMake(0, 0);
    }
...
- (void)setUpUnderLineEffect:(void(^)(BOOL *isUnderLineDelayScroll,CGFloat *underLineH,CGFloat *underLinefixedW,UIColor **underLineColor,BOOL *isUnderLineEqualTitleWidth))underLineBlock
{
    ...
    if (underLineBlock) {
        underLineBlock(&_isDelayScroll,&_underLineH,&_underLinefixedW,&underLineColor,&_isUnderLineEqualTitleWidth);
...
- (void)setUpTitleEffect:(void(^)(UIColor **titleScrollViewColor,UIColor **norColor,UIColor **selColor,UIFont **titleFont,UIFont **titleSelFont,CGFloat *titleHeight,CGFloat *titleWidth))titleEffectBlock{
    ...
    if (titleEffectBlock) {
        titleEffectBlock(&titleScrollViewColor,&norColor,&selColor,&titleFont,&titleSelFont,&_titleHeight,&_titleWidth);
        if (norColor) {
            self.norColor = norColor;
        }
        if (selColor) {
        ...
        if (titleSelFont) {
            self.titleSelFont=titleSelFont;
        }
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = self.navigationController.navigationBarHidden == NO ?([UNApplicationTool getNav_StatusH]):statusH;
...
//    NSUInteger count = self.childViewControllers.count;
    ...
    for (NSString *title in titles) {
        ...
        CGRect titleNomalBounds = [title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
        CGRect titleSelBounds = [title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleSelFont} context:nil];

        CGFloat width = MAX(titleNomalBounds.size.width, titleSelBounds.size.width);
...
//    if (totalWidth > YZScreenW) {
        ...
        _titleMargin = kTransformP(margin);
        
        self.titleScrollView.contentInset = UIEdgeInsetsMake(0, 0, 0, _titleMargin);//+kGetP(33)
...
- (void)setUpAllTitle
{
    ...
    if (_isSHowAdd) {
        // 设置标题滚动视图的内容范围
        UILabel *lastLabel = self.titleLabels.lastObject;
        _addBtn.frame=CGRectMake(  CGRectGetMaxX(lastLabel.frame),lastLabel.x, _titleMargin +kTransformP(18)+_titleMargin, labelH);
        _titleScrollView.contentSize = CGSizeMake(CGRectGetMaxX(_addBtn.frame)+kTransformP(33), 0);
        _addBtn.hidden=NO;
    }else
    {
        // 设置标题滚动视图的内容范围
        UILabel *lastLabel = self.titleLabels.lastObject;
        _titleScrollView.contentSize = CGSizeMake(CGRectGetMaxX(lastLabel.frame)+kTransformP(33), 0);
        _addBtn.hidden=YES;
    }
...
//    CGFloat centerDelta = rightLabel.yz_x - leftLabel.yz_x;
   // 获取两个标题中心点的距离
    ...
    self.underLine.yz_width += self.underLinefixedW>0?0:underLineWidth;
    self.underLine.yz_x += underLineTransformX;
    
//    debugLog(@"-----centerDelta=%f,widthDelta=%f,offsetDelta=%f,,underLineTransformX=%f,underLineWidth=%f------",centerDelta,widthDelta,offsetDelta,underLineTransformX,underLineWidth);
}
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    self.underLine.yz_y = label.yz_height - underLineH;//-kGetP(5);//-kGetP(8)
    ...
    if (self.underLine.yz_x == 0) {
        if (_isUnderLineEqualTitleWidth) {
            ...
            self.underLine.yz_width = self.underLinefixedW>0?self.underLinefixedW:label.yz_width;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    debugLog(@"---------------%@---%@",cell.contentView,vc.view);
