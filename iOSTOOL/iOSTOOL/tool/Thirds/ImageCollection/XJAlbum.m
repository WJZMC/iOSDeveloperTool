...
//        [[SDWebImageManager sharedManager]loadImageWithURL:[NSURL URLWithString:[mImgUrlArr objectAtIndex:i]] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
//            [SVProgressHUD showProgress:receivedSize*1.0f/expectedSize status:@"加载中"];
...
//        }];
//        [[SDWebImageManager sharedManager]downloadImageWithURL: options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
//            ;
...
-(void)addGestureRecognizer
{
    ...
    for (int i =0; i<count; i++) {
        ...
        [singleview addGestureRecognizer:longPressGest];
    }
}
...
- (void)longPressView:(UILongPressGestureRecognizer *)longPressGest{
    ...
    if (longPressGest.state==UIGestureRecognizerStateBegan) {
        ...
        [UNApplicationTool showActionSheetWithConfig:@[@{@"name":@"保存图片",@"font":kFontNomal(16),@"color":kBlackColor},@{@"name":@"取消",@"font":kFontNomal(16),@"color":kBlackColor}] WithBlock:^(NSInteger type) {
            if (type == 0) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if (error == nil) {
        ...
        [UNApplicationTool showAlertWithMsg:@"保存成功" WithParentView:self.view];
    } else {
        
        [UNApplicationTool showAlertWithMsg:@"保存失败" WithParentView:self.view];
    }
}
...
-(void)refushContent
{
    UIScrollView *singleScrollView = (UIScrollView*)[wholeScoll viewWithTag:101+mcurpage];
    UIImageView *imgview = singleScrollView.subviews[0];
    [imgview sd_setImageWithURL:[NSURL URLWithString:[mImgUrlArr objectAtIndex:mcurpage]] placeholderImage:@"" options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        [SVProgressHUD showProgress:receivedSize*1.0f/expectedSize status:@"加载中"];
    } completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        [SVProgressHUD dismiss];
        if (error) {
            [UNApplicationTool showAlertWithMsg:@"图片加载失败" WithParentView:kWindows];
        }
        imgview.frame = [self getRectfixMinImageView:imgview];
        singleScrollView.contentSize = [self getScrollViewContentSize:imgview];
        singleScrollView.contentOffset = CGPointMake(0, 0);
